...
def convert_pytorch_to_onnx(
    ...
    try:
        ...
        # Prepare inputs
        if isinstance(input_sample, (list, tuple)):
            ort_inputs = {name: input_sample[i].cpu().numpy() for i, name in enumerate(input_names)}
        else:
        ...
        if isinstance(torch_outputs, (list, tuple)):
            ...
            np.testing.assert_allclose(torch_out_np, ort_outputs[0], rtol=1e-3, atol=1e-5)
...
if __name__ == "__main__":
    ...
    convert_pytorch_to_onnx(
        ...
        opset_version=14,              # Higher opset for newer operators
        ...
        optimize_onnx=True,            # Apply optimizations
    )